    ...
    onRender: function (item) {
		//changeParser();
			setTimeout(function() {
				...
				/*if (localStorage.getItem('parser_torrent_type') !== 'jackett') {
                                   $('div[data-name="jackett_urltwo"]').hide()
                                }*/
				if(Lampa.Storage.field('parser_torrent_type') == 'jackett')&if(Lampa.Storage.field('parser_use')) item.show()&$('.settings-param__name', item).css('color','ffffff')&$('div[data-name="jackett_urltwo"]').insertAfter('div[data-name="parser_torrent_type"]');
				else item.hide();
			}, 20);
        }
   });
