...
function myRequest(i) {
	setTimeout(function() {
		...
		if (parserBase[i] == '79.137.204.8:9117') var myAdder = '777&Query=Каскадёры&title=Каскадёры&title_original=The%20Fall%20Guy&year=2024&is_serial=1&genres=боевик%2Cкомедия%2Cкриминал%2Cдетектив%2Cмелодрама&Category[]=2000'
		var k = i + 2; 
		...
		xhr.ontimeout = function() {
                ...
                xhr.onload = function() {
                   if (xhr.status == 200) {
                       if ($(mySelector).text() == parserName[i]) $(mySelector).html('&#10004;&nbsp;&nbsp;' + $(mySelector).text()).css('color', '64e364');
		   } else {
	               if ($(mySelector).text() == parserName[i]) $(mySelector).html('&#10008;&nbsp;&nbsp;' + $(mySelector).text()).css('color', 'ff2121');
                   }
                   /*if (xhr.status == 401) {
...
function changeParser() {
     ...
     if (Lampa.Storage.get('jackett_urltwo') == 'bylampa_jackett') 	   Lampa.Storage.set('jackett_url', '79.137.204.8:9117')&Lampa.Storage.set('jackett_key', '777')&Lampa.Storage.set('jackett_interview', 'all')&Lampa.Storage.set('parse_in_search', true)&Lampa.Storage.set('parse_lang', 'df');
	 return;
}
	...
	if (url == '79.137.204.8:9117') var myAdder = '777'
        ...
        xhr.onload = function() {
           ...
           // console.log('Response Text:', xhr.responseText);
            ...
            if (xhr.status === 200) {
                menuItem.title = '<span style="color: #64e364;">&#10004;&nbsp;&nbsp;' + title + '</span>';
                resolve(menuItem);
            } else {
                menuItem.title = '<span style="color: #ff2121;">&#10008;&nbsp;&nbsp;' + title + '</span>';
                resolve(menuItem);
            }
        };
